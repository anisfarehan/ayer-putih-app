...
export class LoginPage {
  ...
  constructor(public globalService:GlobalService, public userData:UserData, public navCtrl:NavController, public auth:AuthService, public user:User, public alertCtrl:AlertController, public loadingCtrl:LoadingController, public pushData:PushDataService) {
      ...
      this.auth.login({'number_ic': this.number_ic, 'password': this.password}).subscribe((isValid:any) => {
        ...
        if (isValid) {
          this.userData.getUserData().then((data:any)=> {
            this.pushData.getDeviceToken().then((data:any)=> {
              this.pushData.updateGCMRegistrationId().subscribe((success:any)=> {}, (error:any)=> {
                console.log(error);
              });

            });

            this.globalService.alert("", "Selamat Datang " + data.number_ic + "!").present();
